    ...
    def _make_request(self, 
        ...
        while retry_count < max_retries:
            try:
                ...
                if pow_required:
                    challenge = self._get_pow_challenge(pow_type)
        ...
        try:
            headers = self._get_headers(
                pow_response=self.pow_solver.solve_challenge(
                    self._get_pow_challenge('/api/v0/chat/completion')
                )
            )
    ...
    def upload_file(self, file_path: str) -> Optional[list[str]]:
        ...
        try:
            ...
            if status == 'SUCCESS':
                return file_id
            if status == 'PENDING':
                return self.fetch_files(file_id)
            return None
        except KeyError:
